...
router.post('/start', authenticateAPIKey, checkSandboxMode, verificationRateLimit, [
    ...
    await stateManager.initializeVerification(verificationRequest.id, user_id, req.isSandbox || false);
            ...
            try {
                ...
                if (document) {
                    ...
                    if (backDocument) {
                        ...
                        const comprehensiveValidationFailed = currentVerification.status === 'failed' &&
...
router.post('/check-consistency/:verification_id', authenticateAPIKey, [
    param('verification_id')
        .isUUID()
        .withMessage('Verification ID must be a valid UUID')
], catchAsync(async (req, res) => {
...
router.post('/reupload-document/:verification_id', authenticateAPIKey, checkSandboxMode, verificationRateLimit, upload.single('document'), [
    param('verification_id')
        .isUUID()
        .withMessage('Verification ID must be a valid UUID'),
    body('document_type')
    ...
    try {
        ...
        const existingDoc = existingDocuments?.find(doc => isBackSide ? doc.is_back_of_id : !doc.is_back_of_id);
        // Store new document file
        const documentPath = await storageService.storeDocument(file.buffer, isBackSide ? `back_${file.originalname}` : file.originalname, file.mimetype, verificationRequest.id);
