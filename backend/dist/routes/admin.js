...
router.get('/dashboard', authenticateJWT, requireAdminRole(['admin', 'reviewer']), catchAsync(async (req, res) => {
    const [verificationStats, recentVerifications, developerCount, systemHealth] = await Promise.all([
...
router.get('/verifications', authenticateJWT, requireAdminRole(['admin', 'reviewer']), [
    query('status')
        .optional()
    ...
    const filters = {
        ...
        developerId: req.query.developer_id,
...
router.get('/verification/:id', authenticateJWT, requireAdminRole(['admin', 'reviewer']), [
    param('id')
        .isUUID()
...
router.put('/verification/:id/review', authenticateJWT, requireAdminRole(['admin', 'reviewer']), [
    param('id')
        .isUUID()
        .withMessage('Verification ID must be a valid UUID'),
    body('decision')
...
// Get developers list
router.get('/developers', authenticateJWT, requireAdminRole(['admin']), [
    query('page')
        .optional()
        .isInt({ min: 1 })
        .withMessage('Page must be a positive integer'),
    ...
    const developersWithStats = await Promise.all(developers.map(async (dev) => {
...
router.get('/analytics', authenticateJWT, requireAdminRole(['admin']), [
    query('period')
        .optional()
        .isIn(['7d', '30d', '90d'])
...
router.get('/health', authenticateJWT, requireAdminRole(['admin']), catchAsync(async (req, res) => {
...
async function getSystemHealth() {
    const [storageHealth, dbHealth, pendingWebhooks] = await Promise.all([
...
async function getPendingWebhookCount() {
    const { count, error } = await supabase
...
async function getAnalytics(period, developerId) {
