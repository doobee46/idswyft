...
router.get('/', authenticateAPIKey, catchAsync(async (req, res) => {
    const developerId = req.developer.id;
...
// Update webhook
router.put('/:webhookId', authenticateAPIKey, [
    body('url')
        .optional()
        .isURL()
        .withMessage('Valid webhook URL is required'),
...
// Delete webhook
router.delete('/:webhookId', authenticateAPIKey, catchAsync(async (req, res) => {
    const { webhookId } = req.params;
    const developerId = req.developer.id;
    // Verify webhook belongs to developer
    const webhook = await webhookService.getWebhookById(webhookId);
...
router.get('/:webhookId/deliveries', authenticateAPIKey, catchAsync(async (req, res) => {
    const { webhookId } = req.params;
    const developerId = req.developer.id;
...
router.post('/:webhookId/test', authenticateAPIKey, catchAsync(async (req, res) => {
    const { webhookId } = req.params;
    const developerId = req.developer.id;
    // Verify webhook belongs to developer
    const webhook = await webhookService.getWebhookById(webhookId);
