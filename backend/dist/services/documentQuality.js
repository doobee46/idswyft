...
export class DocumentQualityService {
    ...
    static MIN_FILE_SIZE = 50 * 1024; // 50KB
    static BLUR_THRESHOLD = 100;
    static MIN_BRIGHTNESS = 50;
    static MAX_BRIGHTNESS = 200;
    static async analyzeDocument(filePath) {
        try {
            ...
            const overallQuality = this.determineOverallQuality(blurScore >= this.BLUR_THRESHOLD, resolution.isHighRes, brightness >= this.MIN_BRIGHTNESS && brightness <= this.MAX_BRIGHTNESS, fileSizeAnalysis.isReasonableSize, contrast);
            return {
    ...
    static calculateImageStats(image) {
    ...
    static calculateBlurScore(image) {
        try {
    ...
    static determineOverallQuality(isSharp, isHighRes, isBrightnessGood, isSizeGood, contrast) {
