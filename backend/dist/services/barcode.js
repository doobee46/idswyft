...
export class BarcodeService {
     ...
     */
    async parsePDF417(rawBarcodeData) {
        try {
            ...
            const populatedFields = Object.values(parsedData).filter(value => value !== null && value !== undefined && value !== '').length;
            ...
            const missingCriticalFields = criticalFields.filter(field => !parsedData[field] || parsedData[field] === '');
     ...
     */
    async scanWithPDF417AndOCR(imagePath) {
        try {
     ...
     */
    async detectPDF417WithOCR(imagePath) {
        try {
     ...
     */
    async detectPDF417WithAI(imagePath) {
        try {
            if (!this.useAiBarcodeReading) {
...
IMPORTANT: Extract the complete raw text data exactly as it appears in the barcode. This data will be parsed separately.`
     ...
     */
    async detectPDF417WithZXing(imagePath) {
        if (!ZXing || !Jimp) {
            ...
            for (const reader of readers) {
                try {
                    ...
                    for (let i = 0; i < grayscaleData.length; i++) {
                        ...
                        grayscaleData[i] = Math.round(0.299 * imageData[pixelIndex] + // R
     ...
     */
    compareHeightFormats(height1, height2) {
        const normalizeHeight = (h) => {
            ...
            if (inchesMatch) {
                ...
                if (inches > 12)
                    return inches; // Assume total inches
...
This is for document verification and security analysis purposes.`
    ...
    async scanWithTraditional(imagePath) {
        try {
            // Download and process image
            const imageBuffer = await this.storageService.downloadFile(imagePath);
            const image = await Jimp.read(imageBuffer);
    ...
    async detectBarcodesInImage(image) {
    ...
    async detectQRCodesInImage(image) {
    ...
    analyzeHorizontalLine(image, startY, endY) {
            ...
            for (let x = 0; x < width - minSquareSize; x += 10) {
                for (let size = minSquareSize; size <= maxSquareSize; size += 10) {
                    ...
                    if (this.isSquarePattern(image, x, y, size)) {
                        ...
                        if (corners.length >= 3)
                            return corners; // QR codes have 3 finder patterns
                    }
                }
            }
        }
    ...
    isSquarePattern(image, startX, startY, size) {
    ...
    async extractTextFromBackOfId(image) {
    ...
    extractVerificationCodes(textData) {
    ...
    compareSections(image, x1, y1, x2, y2, size) {
        let differences = 0;
    ...
    parseAIBarcodeResponse(aiResponse) {
        try {
    ...
    async scanWithLocalOCR(imagePath) {
        try {
    ...
    async preprocessImageForBackOfId(imageBuffer) {
        try {
    ...
    extractBackOfIdStructuredData(ocrText) {
        ...
        for (const pattern of idPatterns) {
            ...
            if (matches) {
                ...
                if (result.parsed_data.id_number)
                    break;
            }
        }
        ...
        for (const pattern of datePatterns) {
            ...
            if (matches) {
                ...
                if (result.parsed_data.expiry_date)
                    break;
            }
        }
        ...
        result.security_features = securityKeywords.filter(keyword => cleanText.toUpperCase().includes(keyword));
    ...
    normalizeDateForComparison(dateStr) {
        ...
        const date = this.parseDate(dateStr);
        if (!date)
            return dateStr.replace(/\D/g, ''); // fallback: remove all non-digits
        const month = (date.getMonth() + 1).toString().padStart(2, '0');
        const day = date.getDate().toString().padStart(2, '0');
        const year = date.getFullYear().toString();
        return `${year}${month}${day}`; // YYYYMMDD format for reliable comparison
    }
    async crossValidateWithFrontId(frontOcrData, backOfIdData) {
        const discrepancies = [];
        let matches = 0;
        let totalChecks = 0;
        ...
        if (frontOcrData?.issuing_authority && backOfIdData.parsed_data?.issuing_authority) {
            totalChecks++;
            const authorityMatch = this.matchIssuingAuthorities(frontOcrData.issuing_authority, backOfIdData.parsed_data.issuing_authority);
            if (authorityMatch)
                matches++;
            else
                discrepancies.push(`Issuing authority mismatch: front="${frontOcrData.issuing_authority}" vs back="${backOfIdData.parsed_data.issuing_authority}"`);
        }
        // PDF417-specific validation and cross-checking
        let pdf417Insights = undefined;
        let nameMatch = undefined;
        let pdf417Validation = 'not_found';
        if (backOfIdData.pdf417_data) {
            ...
            if ((frontFirstName || frontLastName) && (pdf417.firstName || pdf417.lastName)) {
                ...
                if (nameMatch) {
                    ...
                    discrepancies.push(`Name mismatch: front="${frontFirstName} ${frontLastName}" vs PDF417="${pdf417.firstName} ${pdf417.lastName}"`);
                }
            }
            ...
            if (frontIdNumber && pdf417.licenseNumber) {
                ...
                if (pdf417IdNormalized === frontIdNormalized) {
                    ...
                    discrepancies.push(`PDF417 ID mismatch: front="${frontIdNumber}" (${frontIdNormalized}) vs PDF417="${pdf417.licenseNumber}" (${pdf417IdNormalized})`);
                }
            }
            ...
            if (frontDOB && pdf417.dateOfBirth) {
                ...
                if (frontDOBNormalized === pdf417DOBNormalized) {
                    ...
                    discrepancies.push(`PDF417 DOB mismatch: front="${frontDOB}" vs PDF417="${pdf417.dateOfBirth}"`);
                }
            }
            ...
            if (frontAddress && pdf417FullAddress) {
                ...
                if (addressSimilarity > 0.7) {
                    ...
                    discrepancies.push(`PDF417 address mismatch (similarity: ${(addressSimilarity * 100).toFixed(1)}%): front="${frontAddress}" vs PDF417="${pdf417FullAddress}"`);
                }
            }
            ...
            if (frontGender && pdf417.gender) {
                ...
                if (frontGenderNormalized === pdf417GenderNormalized) {
                    ...
                    discrepancies.push(`PDF417 gender mismatch: front="${frontGender}" vs PDF417="${pdf417.gender}"`);
                }
            }
            ...
            if (frontHeight && pdf417.height) {
                ...
                if (frontHeightAAMVA === pdf417HeightAAMVA) {
                    ...
                    discrepancies.push(`PDF417 height mismatch: front="${frontHeight}" (${frontHeightAAMVA}) vs PDF417="${pdf417HeightAAMVA}"`);
                }
            }
            ...
            if (frontExpiryDate && pdf417.expirationDate) {
                ...
                if (pdf417DateNormalized === frontDateNormalized) {
                    ...
                    discrepancies.push(`PDF417 expiry mismatch: front="${frontExpiryDate}" vs PDF417="${pdf417.expirationDate}"`);
                }
            }
            ...
            if (frontEyeColor && pdf417.eyeColor) {
                ...
                if (frontEyeNormalized === pdf417EyeNormalized ||
                    ...
                    discrepancies.push(`PDF417 eye color mismatch: front="${frontEyeColor}" vs PDF417="${pdf417.eyeColor}"`);
                }
            }
            ...
            if (frontWeight && pdf417.weight) {
                ...
                if (Math.abs(frontWeightNum - pdf417WeightNum) <= 5) {
                    ...
                    discrepancies.push(`PDF417 weight mismatch: front="${frontWeight}" vs PDF417="${pdf417.weight}"`);
                }
            }
            ...
            if (frontMiddleName && pdf417.middleName) {
                ...
                if (frontMiddleNormalized === pdf417MiddleNormalized ||
                    ...
                    discrepancies.push(`PDF417 middle name mismatch: front="${frontMiddleName}" vs PDF417="${pdf417.middleName}"`);
                }
            }
            ...
            if (frontVehicleClass && pdf417.vehicleClass) {
                ...
                if (frontClassNormalized === pdf417ClassNormalized) {
                    ...
                    discrepancies.push(`PDF417 vehicle class mismatch: front="${frontVehicleClass}" vs PDF417="${pdf417.vehicleClass}"`);
                }
            }
            ...
            if (frontDocDiscriminator && pdf417.documentDiscriminator) {
                ...
                if (frontDocDiscriminator === pdf417.documentDiscriminator) {
                    ...
                    discrepancies.push(`PDF417 document discriminator mismatch: front="${frontDocDiscriminator}" vs PDF417="${pdf417.documentDiscriminator}"`);
                }
            ...
            if (frontOcrData?.issuing_authority && pdf417.state) {
                ...
                if (this.matchIssuingAuthorities(frontOcrData.issuing_authority, pdf417.state)) {
                    ...
                    discrepancies.push(`PDF417 state mismatch: front authority="${frontOcrData.issuing_authority}" vs PDF417 state="${pdf417.state}"`);
                }
            }
    ...
    matchIssuingAuthorities(authority1, authority2) {
        ...
        for (const [canonical, variants] of Object.entries(authorityMappings)) {
            ...
            const auth1Matches = allVariants.some(variant => auth1.includes(variant) || variant.includes(auth1));
            const auth2Matches = allVariants.some(variant => auth2.includes(variant) || variant.includes(auth2));
