...
export const apiActivityLogger = (req, res, next) => {
    ...
    res.end = function (chunk, encoding) {
        ...
        if (developer?.id && apiKey?.id) { // Only log if there's an API key (external API usage)
            ...
            if (!isDeveloperPortalCall) {
                ...
                activities.unshift({ ...logEntry, timestamp: new Date() });
...
export const getRecentActivities = (developerId) => {
    return recentActivities.get(developerId) || [];
};
// Clear activities for a developer
export const clearActivities = (developerId) => {
