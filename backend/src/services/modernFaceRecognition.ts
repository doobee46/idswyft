...
export class ModernFaceRecognitionService {
  ...
  constructor() {
    ...
    console.log('🎯 Modern Face Recognition Service initialized');
  ...
  private async downloadModelsIfNeeded(): Promise<void> {
    ...
    console.log(`⚠️  Face recognition models not found at: ${this.modelsPath}`);
    console.log('📥 Please download models from: https://github.com/vladmandic/face-api/tree/master/model');
  ...
  private async extractFaceDescriptor(imagePath: string): Promise<FaceDescriptor | null> {
    try {
      ...
      if (!detection) {
        console.log(`❌ No face detected in image: ${path.basename(imagePath)}`);
        return null;
      }

      if (detection.detection.score < this.FACE_DETECTION_SCORE_THRESHOLD) {
        console.log(`❌ Face detection confidence too low: ${detection.detection.score} < ${this.FACE_DETECTION_SCORE_THRESHOLD}`);
        return null;
      }
  ...
  async compareFaces(imagePath1: string, imagePath2: string): Promise<number> {
    ...
    console.log('🔍 Starting modern face comparison...');
    ...
    try {
      ...
      console.error('❌ Modern face comparison failed:', error);
  ...
  async detectLiveness(imagePath: string): Promise<number> {
    ...
    console.log('👁️  Starting modern liveness detection...');
    ...
    try {
      ...
      if (!detection) {
        console.log('❌ No face detected for liveness analysis');
        return 0.0;
      }
      ...
      console.log(`👁️  Liveness Detection Results:`);
      console.log(`   📊 Overall Score: ${livenessScore.toFixed(4)}`);
      console.log(`   🎯 Face Detected: ${detection.detection.score.toFixed(3)}`);
      
      if (detection.expressions) {
        const topExpression = Object.entries(detection.expressions)
          .sort(([,a], [,b]) => b - a)[0];
        console.log(`   😊 Top Expression: ${topExpression[0]} (${topExpression[1].toFixed(3)})`);
      }
      ...
      console.error('❌ Modern liveness detection failed:', error);
  ...
  private calculateLivenessScore(detection: any, image: any): number {
    ...
    console.log(`   🎯 Face Confidence: ${faceConfidence.toFixed(3)} (weight: 0.3)`);
    ...
    if (detection.landmarks) {
      ...
      factorCount += 0.25;
      console.log(`   🗺️  Landmark Quality: ${landmarkQuality.toFixed(3)} (weight: 0.25)`);
    }
    ...
    if (detection.expressions) {
      ...
      factorCount += 0.2;
      console.log(`   😊 Expression Naturalness: ${expressionNaturalness.toFixed(3)} (weight: 0.2)`);
    }
    ...
    if (detection.age && detection.gender) {
      ...
      factorCount += 0.15;
      console.log(`   👤 Demographic Score: ${demographicScore.toFixed(3)} (weight: 0.15)`);
      console.log(`   📅 Detected Age: ${Math.round(detection.age)}, Gender: ${detection.gender}`);
    }
    ...
    factorCount += 0.1;
    console.log(`   📷 Image Quality: ${imageQuality.toFixed(3)} (weight: 0.1)`);

    // Normalize score
    const normalizedScore = factorCount > 0 ? score / factorCount : 0;
    const finalScore = Math.max(0, Math.min(1, normalizedScore));

    console.log(`   📊 Weighted Score: ${score.toFixed(3)} / ${factorCount.toFixed(2)} = ${finalScore.toFixed(3)}`);
   ...
   */
  private assessLandmarkQuality(landmarks: faceapi.FaceLandmarks68): number {
   ...
   */
  private assessExpressionNaturalness(expressions: faceapi.FaceExpressions): number {
    ...
    const expressionVariety = expressionValues.filter(v => v > 0.1).length;
