...
Return JSON analysis with:
{
   ...
   */
  private async detectFacesWithAI(imageBuffer: Buffer): Promise<FaceDetectionResult> {
    const base64Image = imageBuffer.toString('base64');
    const mimeType = this.detectMimeType(imageBuffer);
...
Return JSON with:
{
   ...
   */
  private async detectFacesWithImageAnalysis(imageBuffer: Buffer): Promise<FaceDetectionResult> {
  ...
  private parseAIFaceResponse(response: string): any {
    try {
  ...
  private parseAILivenessResponse(response: string): any {
    try {
      // Extract JSON from response
      const jsonMatch = response.match(/\{[\s\S]*\}/);
      if (jsonMatch) {
