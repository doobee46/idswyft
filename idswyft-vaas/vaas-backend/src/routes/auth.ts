...
import { VaasApiResponse, VaasLoginRequest, VaasLoginResponse, VaasEnterpriseSignupRequest } from '../types/index.js';
...
// Enterprise signup - public endpoint for businesses to register
router.post('/signup', async (req, res) => {
  try {
    const {
      ...
      company_size,
      industry
    }: VaasEnterpriseSignupRequest = req.body;
    ...
    if (!company_name || !admin_email || !admin_password || !admin_first_name || !admin_last_name) {
      const response: VaasApiResponse = {
        ...
        error: {
          ...
          message: 'Company name, admin email, password, first name, and last name are required'
        }
      };
    ...
    const { data: organization, error: orgError } = await vaasSupabase
      ...
      .insert({
        name: company_name,
        slug: orgSlug,
        contact_email: company_email || admin_email,
        contact_phone: phone_number || null,
        ...
        settings: {
          ...
          verification_credits: 1000 // Free trial credits in settings
        }
      })
      .select()
      .single();
    ...
    if (adminError || !admin) {
      // Cleanup: delete the organization if admin creation failed
      await vaasSupabase
        .from('vaas_organizations')
        .delete()
    ...
    console.log(`Welcome email for ${admin.email}: ${config.frontendUrl}/verify-email?token=${verificationToken}`);
    ...
    const response: VaasApiResponse = {
      ...
      data: {
        ...
        organization: {
          ...
          verification_credits: (organization as any).settings?.verification_credits || 1000
        },
        admin: {
          id: admin.id,
          email: admin.email,
    ...
    const response: VaasApiResponse = {
      ...
      error: {
        code: 'SIGNUP_FAILED',
        message: 'Failed to create enterprise account. Please try again.'
      }
    };
